...
import { TicketUserConversation } from './ticket-conversation';
...
isLoading,
  ...
  queryKey: ["ticket", id],
  queryFn: () => apiServices.ticket.detailsByUser(id), // Use non-null assertion if confident
  ...
  return (
    <div dir="rtl" className="space-y-6">
      <Card>
        ...
        <h2 className="text-2xl font-bold text-foreground">مکالمه </h2>
        <Badge className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white shadow-lg">{appointment.data.status}</Badge>
      </div>
        </CardContent>
      </Card>

      {id && <TicketUserConversation onRefetch={refetch} data={appointment.data}   />}
    </div>
  );
};

export default TicketUserDetailsPage;
